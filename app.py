...
def main():
  ...
  while True:
    led ^= cwiid.LED4_ON
    led = led & (led ^ cwiid.LED2_ON) 
    res = system("./check_ip_address")
    if res == 0:
      led = led | cwiid.LED3_ON
    else:
      led = led & (led ^ cwiid.LED3_ON) 
    wiimote.led = led
   
    
    print_state(wiimote.state)
    now = time.time()
    if (now - start_time > (5*60)) or controller.left_pressed():
      controller.rumble(0.1)
      start_time = now
      res = system("./snapshot.sh")
      if res != 0:
        controller.rumble(0.1)
        raise NameError("tweet command returned " + str(res))
      else:
        led = led | cwiid.LED2_ON
        wiimote.led = led
        system("./tweet-last-snapshot.sh")
        controller.rumble(1)
